...
def run_prediction(request, session_id=None, display_mri=None):
...
def run_segmentation(request, session_id=None, display_mri=None):
    if request.method == 'GET':
        file_url = request.GET.get('file_url', None)
        if file_url is not None:
...
def remove_tmp(request, session_id=None, display_mri=None):
    if request.method == 'GET':
