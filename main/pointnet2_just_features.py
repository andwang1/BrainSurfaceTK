...
from src.data_loader_just_features import OurDataset
...
class GlobalSAModule(torch.nn.Module):
    ...
    def forward(self, x, pos, batch, numb_features):
        x = self.nn(torch.cat([x, pos], dim=1))
        x = global_max_pool(x, batch)
        pos = pos.new_zeros((x.size(0), numb_features)) # TODO
...
class Net(torch.nn.Module):
    def __init__(self, num_local_features, num_global_features):
        ...
        self.sa1_module = SAModule(0.5, 0.2, MLP([num_local_features, 64, 64, 96]))
        self.sa1a_module = SAModule(0.5, 0.2, MLP([96 + num_local_features, 96, 96, 128]))
        self.sa2_module = SAModule(0.25, 0.4, MLP([128 + num_local_features, 128, 128, 256]))
        self.sa3_module = GlobalSAModule(MLP([256 + num_local_features, 256, 512, 1024]))
    ...
    def forward(self, data):
        ...
        sa3_out = self.sa3_module(*sa2_out, self.num_local_features)
        x, pos, batch = sa3_out
