...
def get_all_unique_labels(meta_data, vtk_path, extension):
    '''
    ...
    for idx, patient_id in enumerate(tqdm(meta_data[:, 0])):            # Get file path to .vtk/.vtp for one patient
        file_path = get_file_path(vtk_path, patient_id, meta_data[idx, 1], extension)
        # If file exists
