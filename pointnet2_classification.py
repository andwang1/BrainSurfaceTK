...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.lin3 = Lin(256, 2)  # OUTPUT = NUMBER OF CLASSES, 1 IF REGRESSION TASK
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        loss = F.nll_loss(model(data), data.y[:, 0].long())
...
if __name__ == '__main__':
    ...
    # Additional comments
    comment = "Numb_Points_15000_CLASSIFICATION"
    ...
    train_dataset = OurDataset(path, label_class='gender', train=True, classification=True,
                                 transform=transform, pre_transform=pre_transform, add_birth_weight=add_birth_weight)
    test_dataset = OurDataset(path, label_class='gender', train=False, classification=True,
                                 transform=transform, pre_transform=pre_transform, add_birth_weight=add_birth_weight)
    ...
    for epoch in range(1, 101):
        ...
        test_acc = test_classification(test_loader)
